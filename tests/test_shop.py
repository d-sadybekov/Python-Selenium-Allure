...
def test_top_menu(browser):
    ...
    Test case TC-2
    """
    ### Убрал пункт меню "Часто задаваемые вопросы"
    expected_menu = ['Каталог', 'Блог', 'О компании', 'Контакты']
    browser.get(URL)
    ### поменял айди с menu-top на menu-primary-menu
...
def test_right_way(browser):
    ...
    WebDriverWait(browser, timeout=10, poll_frequency=2).until(EC.text_to_be_present_in_element(
        (By.CLASS_NAME, "found-post"), "16"))

    product = browser.find_element(by=By.CSS_SELECTOR, value="[data-product_sku='MR544OL1PR']")
    product.click()
    ...
    WebDriverWait(browser, timeout=15, poll_frequency=1).until(
        EC.url_changes("https://testqastudio.me/checkout/"))
